...
def dump_tabular(load_policy=None, *args, **kwargs):
    ...
    if len(_tabular) > 0:
        ...
        for tabular_fd in list(_tabular_fds.values()):
            ...
            if (wh or (wh is None and tabular_fd not in _tabular_header_written)) and (load_policy is None):
                writer.writeheader()
...
def save_itr_params(itr, params, postfix=""):
    if _snapshot_dir:
        if _snapshot_mode == 'all':
            file_name = osp.join(_snapshot_dir, 'itr_'+str(itr)+postfix+'.pkl')
            joblib.dump(params, file_name, compress=3)
        elif _snapshot_mode == 'last':
            # override previous params
            file_name = osp.join(_snapshot_dir, 'params'+postfix+'.pkl')
            joblib.dump(params, file_name, compress=3)
