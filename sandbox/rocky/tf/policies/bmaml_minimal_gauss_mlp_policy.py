...
class BMAMLGaussianMLPPolicy(StochasticPolicy, Serializable):
    def __init__(
            self,
            name,
            env_spec,
        ...
        if mean_network is None:
            self.all_params = self.create_MLP(  # TODO: this should not be a method of the policy! --> helper
                name="mean_network" + str(self.particle_idx),
        ...
        if std_network is not None:
            ...
            if adaptive_std:
                ...
                self.all_params['std_param'] = make_param_layer(
                    ...
                    name="output_std_param" + str(self.particle_idx),
                    trainable=learn_std,
                )
            ...
            super(BMAMLGaussianMLPPolicy, self).__init__(env_spec)
    ...
    def compute_grads(self, num_tasks):
        ...
        for i in range(num_tasks):
            ...
            grads_list.append([gradients[key] for key in update_param_keys])
            params_list.append([self.all_params[key] for key in update_param_keys])
        return grads_list, params_list

    def compute_updated_dists_first(self, grads_list, num_tasks):
        ...
        for i in range(num_tasks):
            gradients = dict(zip(update_param_keys, grads_list[i]))
            fast_params_tensor = OrderedDict(zip(update_param_keys, [self.all_params[key] - self.step_size*gradients[key] for key in update_param_keys]))
    ...
    def get_mean_logstd(self, observation, num_tasks, idx):
        observations = []
        for i in range(num_tasks):
            observations.extend(observation)
        res = self._cur_f_dist(observations)
        means, log_stds = res[idx]
    ...
    def get_params_internal(self, all_params=False, **tags):
        ...
        params = [p for p in params if p.name.startswith('mean_network'+str(self.particle_idx)+"/") or p.name.startswith('output_std_param'+str(self.particle_idx)+"/")]
