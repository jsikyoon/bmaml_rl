...
class BMAMLVPG(BatchBMAMLPolopt, Serializable):
    """
    ...
    def __init__(
        ...
        self.opt_info = None
        super(BMAMLVPG, self).__init__(env=env, policy_list=policy_list, baseline_list=baseline_list, **kwargs)
    ...
    def init_opt(self):
        ...
        for n, self.policy, self.optimizer in zip(range(len(self.policy_list)), self.policy_list, self.optimizer_list):
            ...
            for i in range(self.meta_batch_size):
                ...
                logli = dist_list[n].log_likelihood_sym(action_vars[i], dist_info_vars)
                surr_objs.append(- tf.reduce_mean(logli * adv_vars[i]))
            surr_obj = tf.reduce_mean(tf.stack(surr_objs, 0))
            self.optimizer.update_opt(loss=surr_obj, target=self.policy, inputs=input_list + obs_vars + action_vars + adv_vars) 
        #################################
        return dict()    
