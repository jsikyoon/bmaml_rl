...
class BMAMLCHASER(BatchBMAMLPolopt, Serializable):
    ...
    def __init__(
        ...
        self.opt_info = None
        super(BMAMLCHASER, self).__init__(env=env, policy_list=policy_list, baseline_list=baseline_list, **kwargs)
    ...
    def init_opt(self):
        ...
        for i in range(self.meta_batch_size):
            for n in range(len(self.policy_list)):
                ...
                leader_params_flat = tf.concat([tf.reshape(new_params_list[i][n][key],[-1]) for key in param_keys],axis=0)
                chaser_loss += tf.norm(tf.stop_gradient(leader_params_flat) - follower_params_flat)
        chaser_loss = chaser_loss / self.meta_batch_size
        
        for self.policy, self.optimizer in zip(self.policy_list, self.optimizer_list):
            self.optimizer.update_opt(loss=chaser_loss, target=self.policy, inputs=input_list)
        ############################################################
